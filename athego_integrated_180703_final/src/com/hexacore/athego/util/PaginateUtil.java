...
public class PaginateUtil {
	...
	public static String getPaginate(int pageNo, int total, int numPage, int numBlock, String url) {
		...
		if (total != 0) {
			...
			if (pageNo <= 1) {
				// 비활성화
				paginate += "<span title=\"이전 페이지 없음\"><i class=\"fas fa-caret-left\"></i></span>";
			} else {
				// 활성화
				paginate += "<a href='" + url + (pageNo - 1)
						+ "' title='이전 페이지로'><i class='fas fa-caret-left'></i><span class='screen_out'>이전 페이지</span></a>";
			...
			if (pageNo >= totalPage) {
				// 비활성화
				paginate += "<span title='다음 페이지 없음'><i class='fas fa-caret-right'></i></span>";
			} else {
				// 활성화
				paginate += "<a href='" + url + (pageNo + 1)
						+ "' title='다음 페이지로'><i class='fas fa-caret-right'></i><span class='screen_out'>다음 페이지</span></a>";
			} // if end
	...
	public static String getPaginate(int pageNo, int total, int numPage, int numBlock, String url, String param) {

		return getPaginate(pageNo, total, numPage, numBlock, url + "?" + param);
